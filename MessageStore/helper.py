...
def send_telegram(_case, chat_id, lang, device, item):
    try:
        ...
        match _case:
            case 'gas':
                x = requests.get(
                    'https://api.telegram.org/bot{0}/sendMessage?chat_id={1}&text={2}&parse_mode=HTML'.format(
                        TOKEN, chat_id,
                        '<b>{0} ({2})</b>\r\n{1} {3}%'.format(device.name, language.gasWarning, device.phone,
                                                              item.gas)))
            case 'vol':
                x = requests.get(
                    'https://api.telegram.org/bot{0}/sendMessage?chat_id={1}&text={2}&parse_mode=HTML'.format(
            ...
            case 't1':
                x = requests.get(
                    'https://api.telegram.org/bot{0}/sendMessage?chat_id={1}&text={2}&parse_mode=HTML'.format(
                        TOKEN, chat_id,
                        '<b>{0} ({2})</b>\r\n{1} {3}°C'.format(device.name, language.tempWarning, device.phone,
                                                               item.temperature1)))
            case 't2':
                x = requests.get(
                    'https://api.telegram.org/bot{0}/sendMessage?chat_id={1}&text={2}&parse_mode=HTML'.format(
...
def save_data_v1(body, device):
    # delete old records for better query performance
    try:
        items = Data.objects.filter(device=device, datetime__lte=timezone.now() - timedelta(days=60))
        items.delete()
        datas = Data.objects.filter(device=device, datetime__minute__gt=0)
        datas.delete()
    ...
    try:
        ...
        if datetime.now().minute() == 0:
            item = Data.objects.create(temperature1=data['T1'], humidity1=data['H1'], temperature2=data['T2'],
                                       humidity2=data['H2'],
                                       wind=data['wind'], gas=data['gas'], voltage=float(data['voltage']),
                                       machineStatus=float(data['machine']), mode=float(data['mode']),
                                       signal=float(data['signal']), volBat=float(data['vbat']),
                                       temperatureH1=float(data['TH1']), temperatureH2=float(data['TH2']),
                                       device=device)
        item.save()
        device.active = 2
        device.save()
        ...
        for client in clients:
            if int(client.alertStatus) == 1:
                ...
                if float(data['machine']) < client.machineVoltage and client.lastmessage == 1:
                    ...
                    client.save()
                if int(data['T1']) < client.temperatureAlert and float(
                        data['machine']) < client.machineVoltage and client.lastmessage == 1:
                    send_telegram('machine_not_start',
                                  client.chat_id, client.language, device, item)
                if int(data['T1']) < client.temperatureAlert and float(
                        data['machine']) < client.machineVoltage and client.lastmessage == 0:
...
def save_data(body, device):
    ...
    try:
        ...
        item = Data.objects.create(temperature1=data['T1'], humidity1=data['H1'], temperature2=data['T2'],
                                   humidity2=data['H2'],
                                   wind=data['wind'], gas=data['gas'], voltage=float(data['voltage']),
                                   machineStatus=float(data['machine']), device=device)
        item.save()
        if (device.timeStart.day - datetime.now().day < 0):
            device.timeActive = 0
            device.save()
...
def general_state(item, name):
    try:
        ...
        if float(item.machineStatus) < 12:
            ...
            machine = "ON"
        result = '<b>{0} ({18})</b>\r\n{9}: {1}°C \r\n{10}: {2}% RH\r\n{11}: {3}°C  \r\n{12}: {4}% RH\r\n{13}: {5}  Km/hr\r\n{14}: {6}%\r\n{15}: {7}\r\n{16}:{8}V\r\n<i>{17}</i>"\r\n'.format(
            item.device.name, item.temperature1, item.humidity1, item.temperature2, item.humidity2, item.wind, item.gas,
            machine, item.voltage, lang.temperature1, lang.humidity1, lang.temperature2, lang.humidity2, lang.wind,
            lang.gas, lang.machineStatus, lang.voltage, '{0} GMT+0'.format(item.datetime), item.device.phone)
        return result
    except:
        return 'Not Found'
...
def temperature(item, name):
    try:
        lang = Language.objects.get(name=name)
        result = '<b>{0} ({10})</b>\r\n{5}: {1}°C \r\n{6}: {2}% RH\r\n{7}: {3}°C\r\n{8}: {4}% RH\r\n<i>{9}</i>"\r\n'.format(
            item.device.name, item.temperature1, item.humidity1, item.temperature2, item.humidity2, lang.temperature1,
            lang.humidity1, lang.temperature2, lang.humidity2, '{0} GMT+0'.format(item.datetime), item.device.phone)
        return result
    except:
        return 'Not Found'
